...
//        0, NULL, &outer_source_event);
   ...
   size_t global[3] = {rankinfo->nx, rankinfo->ny, rankinfo->nz};
    ...
    err = meta_gen_opencl_outer_source_calc_outer_source(context->queue, global, local, rankinfo->nx, rankinfo->ny, rankinfo->nz, problem->ng, problem->cmom, problem->nmom, &buffers->fixed_source, &buffers->scattering_matrix, &buffers->scalar_flux, &buffers->scalar_flux_moments, &buffers->outer_source, 1, &outer_source_event);
...
//        0, NULL, &inner_source_event);
   ...
   size_t global[3] = {rankinfo->nx, rankinfo->ny, rankinfo->nz};
    size_t local[3] = {0,0,0};
    err = meta_gen_opencl_inner_source_calc_inner_source(context->queue, global, local, rankinfo->nx, rankinfo->ny, rankinfo->nz, problem->ng, problem->cmom, problem->nmom, &buffers->outer_source, &buffers->scattering_matrix, &buffers->scalar_flux, &buffers->scalar_flux_moments, &buffers->inner_source, 1, &inner_source_event);
